...
# goal 2: match physicians to NPI numbers ####

executive_names <- executive_data %>% ungroup() %>%
  mutate(doctor = ifelse(title %in% c("md", "dr", "do"), 1, NA))

# Filter only to physicians
npidata <- npidata %>%
  filter(t_group=="Allopathic & Osteopathic Physicians"|t_group=="Other Service Providers")
...
executive_names <- executive_names %>%
  mutate(npi=ifelse(ein=="10198331" & name=="katheryn resinbrink", 1124067194, npi),
         ...
         npi=ifelse(ein=="230880420" & name=="thomas curry", 1386712719, npi),
         npi=ifelse(ein=="231352222" & name=="peter hartmann", 1669415691, npi),
         npi=ifelse(ein=="231352222" & name=="steven delavens", 1902801350, npi),
         ...
         npi=ifelse(ein=="741152597" & name=="susan curling", 1467567768, npi),
         ...
         npi=ifelse(ein=="540696355" & name=="stephen krenytzky", 0, npi),
...
# goal 3: identify whether changes occur from year to year ####

executive_data <- executive_names
...
doctors_over_time <- doctors_over_time %>%
  ...
  ungroup() %>%
  mutate(num_doc_change = num_doctors-lag_num_doctors) %>%
  mutate(any_doc_change = ifelse((num_doctors>0 & lag_num_doctors==0) | (num_doctors==0 & lag_num_doctors>0),1,0)) %>%
  select(ein, year, num_doctors, num_doc_change, any_doc_change) %>%
  mutate(hire = ifelse(num_doc_change>0,1,0),
         fire = ifelse(num_doc_change<0,1,0))

executive_data <- executive_data %>%
  left_join(doctors_over_time, by=c("ein", "year")) %>%
  mutate(no_num_md_change_2010_2014 = ifelse(num_doc_change!=0 & year %in% c(2010,2011,2012,2013,2014),1,NA)) %>%
  mutate(no_md_change_2010_2014 = ifelse(any_doc_change!=0 & year %in% c(2010,2011,2012,2013,2014),1,NA)) %>%
  group_by(ein) %>%
...
detect_changes <- executive_data %>% 
  ...
  group_by(ein, year) %>%
  mutate(ceo_md = ifelse(sum(ceo_md, na.rm=T)>0, 1,0)) %>%
  ungroup() %>%
  distinct(ein, year, ceo_md) %>%
  arrange(year) %>%
  group_by(ein) %>%
  mutate(lag_ceo_md = dplyr::lag(ceo_md)) %>%
  ungroup() %>%
  mutate(ceo_md_change = ifelse(ceo_md!=lag_ceo_md,1,0))


# merge back to executive data
executive_data <- executive_data %>%
  left_join(detect_changes, by=c("ein", "year")) %>%
  mutate(no_ceo_change_2010_2014 = ifelse(ceo_md_change==1 & year %in% c(2010,2011,2012,2013,2014),1,NA)) %>%
  group_by(ein) %>%
...
# look at types of physicians
phys_spec <- unique(executive_data$t_class)


# fix some NPIs that aren't classified as physicians in the NPI data
executive_data <- executive_data %>%
  mutate(t_class = ifelse(npi==1730276379, "Internal Medicine", t_class),
         npi = ifelse(npi==1003986944,0,npi),
...
# fill specialties for those I can find that I couldn't match NPI
executive_data <- executive_data %>%
  mutate(t_class = ifelse(name=="melinda estes" & npi==0, "Neurology", t_class),
         t_class = ifelse(name=="jack cooper" & npi==0, "Orthopaedic Surgery", t_class),
         t_class = ifelse(name=="michael rosenberg" & npi==0, "Plastic Surgery", t_class),
         t_class = ifelse(name=="dale adamson" & npi==0, "Family Medicine", t_class),
         t_class = ifelse(name=="dennis 'connor" & npi==0, "Family Medicine", t_class),
         t_class = ifelse(name=="dennis connor" & npi==0, "Family Medicine", t_class),
         t_class = ifelse(name=="bruce cusenz" & npi==0, "Plastic Surgery", t_class),
         t_class = ifelse(name=="david hughes" & npi==0, "Emergency Medicine", t_class),
         t_class = ifelse(name=="jeffrey folk" & npi==0, "Internal Medicine", t_class),
         t_class = ifelse(name=="robert mackenzie" & npi==0, "Surgery", t_class),
         t_class = ifelse(name=="vincent glielmi" & npi==0, "Specialist", t_class),
         t_class = ifelse(name=="charles mackenzie" & npi==0, "Surgery", t_class),
         t_class = ifelse(name=="gregory sheffo" & npi==0, "Pediatrics", t_class),
         t_class = ifelse(name=="wayne munro" & npi==0, "Gastroenterology", t_class),
         t_class = ifelse(name=="robert nelson" & npi==0, "Surgery", t_class),
         t_class = ifelse(name=="robert marshall" & npi==0, "Occupational Medicine", t_class),
         t_class = ifelse(name=="james probst" & npi==0, "Anesthesiology", t_class),
         t_class = ifelse(name=="jason white" & npi==0, "Emergency Physician", t_class),
         t_class = ifelse(name=="brian sears" & npi==0, "Orthopaedic Surgery", t_class),
         t_class = ifelse(name=="dean gruner" & npi==0, "Family Medicine", t_class),
         t_class = ifelse(name=="mark siegel" & npi==0, "Surgery", t_class),
         t_class = ifelse(name=="christopher colenda" & npi==0, "Psychiatry", t_class),
         t_class = ifelse(name=="brad mock" & npi==0, "Anesthesiology", t_class),
         t_class = ifelse(name=="mark peters" & npi==0, "Family Medicine", t_class),
         t_class = ifelse(name=="nancy woodruff" & npi==0, "Family Medicine", t_class),
         t_class = ifelse(name=="richard rodriguez" & npi==0, "Internal Medicine", t_class),
         t_class = ifelse(name=="mitchell rubinstein" & npi==0, "Surgery", t_class),
         t_class = ifelse(name=="anthony granato" & npi==0, "Internal Medicine", t_class),
         t_class = ifelse(name=="daniel brooks" & npi==0, "Surgery", t_class),
         t_class = ifelse(name=="danny bailey" & npi==0, "Surgery", t_class),
         t_class = ifelse(name=="george brown" & npi==0, "Internal Medicine", t_class),
         t_class = ifelse(name=="robert corwin" & npi==0, "Pediatrics", t_class),
         t_class = ifelse(name=="mark sebastian" & npi==0, "Surgery", t_class),
         t_class = ifelse(name=="daniel silverman" & npi==0, "Internal Medicine", t_class),
         t_class = ifelse(name=="thomas mclilwain" & npi==0, "Psychiatry", t_class),
         t_class = ifelse(name=="barbara carbone" & npi==0, "Anesthesiology", t_class),
         t_class = ifelse(name=="alan bier" & npi==0, "Internal Medicine", t_class),
         t_class = ifelse(name=="james raymond" & npi==0, "Internal Medicine", t_class),
         t_class = ifelse(name=="stephen smith" & npi==0, "Internal Medicine", t_class),
         t_class = ifelse(name=="thomas mcgill" & npi==0, "Internal Medicine", t_class),
         t_class = ifelse(name=="denise williams" & npi==0, "Obstetrics & Gynecology", t_class),
         t_class = ifelse(name=="michael buckley" & npi==0, "Internal Medicine", t_class),
         t_class = ifelse(name=="william vandervliet" & npi==0, "Family Medicine", t_class),
         t_class = ifelse(name=="herbert doubek" & npi==0, "Family Medicine", t_class),
         t_class = ifelse(name=="gary yates" & npi==0, "Family Medicine", t_class),
         t_class = ifelse(name=="eugene washington" & npi==0, "Preventative Medicine", t_class),
...
executive_data <- executive_data %>%
  mutate(int_med_doc = ifelse(t_class=="Internal Medicine",1,0)) %>%
  group_by(year, ein) %>%
  mutate(num_int_med_doctors = sum(int_med_doc,na.rm=T)) %>%
  ungroup()

# create md ceo variable
executive_data <- executive_data %>%
  mutate(ceo_md=ifelse(doctor==1 & ceo==1,1,0)) %>%
  group_by(ein) %>%
  mutate(ever_ceo_md=sum(ceo_md, na.rm=T)) %>%
  ungroup()
...
rm(MDPPAS2009, MDPPAS2010, MDPPAS2011, MDPPAS2012, MDPPAS2013, MDPPAS2014, MDPPAS2015, MDPPAS2016, MDPPAS2017)

MDPPAS <- MDPPAS %>%
  mutate(birth_year=substr(birth_dt,start=6, stop=9)) %>%
  mutate(birth_year=as.numeric(birth_year)) %>%
  mutate(age=Year-birth_year) %>%
  select(npi, Year, age, sex, npi_unq_benes, spec_prim_1_name, spec_prim_2_name)

executive_data <- executive_data %>%
  left_join(MDPPAS, by=c("npi","year"="Year"))
...
executive_data <- executive_data %>%
  left_join(PhysCompare, by=c("npi"="NPI"))

doc_exec_data <- executive_data %>%
  filter(doctor==1) %>%
  mutate(ever_cmo = ifelse(cmo==1,1,NA),
         ever_ceo = ifelse(ceo==1,1,NA)) %>%
  group_by(ein, name) %>%
  mutate(min_age = ifelse(year==min(year),age,NA)) %>%
  fill(min_age, sex, grad_year, ever_cmo, ever_ceo, .direction="downup") %>%
  ungroup() %>%
  distinct(name, npi, ever_ceo, ever_cmo, t_class, int_med_doc, min_age, sex, grad_year)
...
doc_exec_data <- executive_data %>%
  ...
  mutate(id=cur_group_id()) %>%
  ungroup() %>%
  mutate(npi=ifelse(npi==0,id,npi)) %>%
  mutate(ever_cmo = ifelse(cmo==1,1,NA),
         ever_ceo = ifelse(ceo==1,1,NA)) %>%
  group_by(npi) %>%
  mutate(min_age = ifelse(year==min(year),age,NA)) %>%

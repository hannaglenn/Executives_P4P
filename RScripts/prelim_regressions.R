...
hospital_data <- readRDS(paste0(created_data_path, "penalized_hospital_data.rds"))
...
regression_data <- hospital_data %>%
  ...
  select(ID, year, heartattack_mortality, heartattack_readmission, pneum_mortality, pneum_readmission, heartfailure_mortality, heartfailure_readmission, has_any_md, minyr_pen) %>%
...
ggplot(graph_did_read, aes(x=year, y=rate, color=outcome, linetype=has_any_md)) + geom_vline(xintercept = 2012) + geom_line(size=.75) + theme_bw() + 
  ylim(15,30) + ggtitle("average readmission rates over time")

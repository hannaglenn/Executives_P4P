...
mort2008 <- mort2008 %>%
  select(provider, condition, measurename, rate) %>%
  mutate(year=2008) %>%
...
mort2009 <- mort2009 %>%
  ...
  mutate(rate=as.numeric(rate))
mort2010 <- mort2010 %>%
  select(provider, condition, measurename, mortality_readmrate)  %>%
  mutate(year=2010) %>%
...
mort2012 <- mort2012 %>%
  ...
  mutate(rate=as.numeric(rate))
mort2013 <- mort2013 %>%
  select(provider, condition, measurename, morreadr)  %>%
  mutate(year=2013) %>%
  rename(providerid=provider, rate=morreadr)  %>%
  mutate(rate=ifelse(rate=="Not Available", NA, rate)) %>%
  mutate(rate=as.numeric(rate))
mort2014 <- mort2014 %>%
  select(providerid, measurename, score)  %>%
...
mortality_data <- mortality_data %>%
  ...
  select(providerid, year, condition, rate_type, rate)

mortality_data <- mortality_data %>%
  pivot_wider(id_cols=c("providerid", "year"), names_from = c("condition", "rate_type"), values_from = "rate")

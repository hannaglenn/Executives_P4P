...
# read in AHA_ein_list_errors1 (scraped text data from manual pdfs)
...
positions_list <- c('vice president', 'peesident', 'president', 'vice chairman', 'chairman','vice chair', 'chairperson', 'chair', 'vice-chair', 'physician', 
                    ...
                    'pres\\b', 'division chief', 'vp', 'cpo', 'coo\\b', 'cmo', 'cno', 
                    'system executive',
...
AHA_ein_data_frame <- rbind(cleaned_text, AHA_ein_data_frame)

saveRDS(AHA_ein_data_frame, paste0(created_data_path, "cleaned_text_notfinished.rds"))

write.csv(cleaned_text, "print.csv")

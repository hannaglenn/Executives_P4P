...
% Affiliation
\node[below=0.15cm] (affiliation) at (author.south){\large The University of Queensland};
% Date
\node[below=0.5cm] (date) at (affiliation.south){\today};
\end{tikzpicture}
...
\begin{columns}[T]
    ...
    \textbf{Characteristics:}
        \begin{itemize}
            \item \tikz[baseline]{\node[anchor=base] (L1) {Gender};}
            \item \tikz[baseline]{\node[anchor=base] (L2) {Experience};}
            \item \tikz[baseline]{\node[anchor=base] (L3) {Age};}
        \end{itemize}
    \end{column}
    \begin{column}{0.45\textwidth}
    \textbf{Firm Outcomes:}
        \begin{itemize}
            \item \tikz[baseline]{\node[anchor=base] (R1) {Value};}
            \item \tikz[baseline]{\node[anchor=base] (R2) {Employees};}
            \item \tikz[baseline]{\node[anchor=base] (R3) {M\&A};}
            \item \tikz[baseline]{\node[anchor=base] (R4) {Leader Salaries};}
        \end{itemize}
    \end{column}
\end{columns}

% Now draw arrows between the nodes
\begin{tikzpicture}[overlay, remember picture]
    \draw[->, thick, shorten >=15pt] (L1.east) to [out=0, in=180] (R1.west);
    \draw[->, thick, shorten >=15pt] (L1.east) to [out=0, in=180] (R2.west);
    \draw[->, thick, shorten >=15pt] (L2.east) to [out=0, in=180] (R3.west);
    \draw[->, thick, shorten >=15pt] (L2.east) to [out=0, in=180] (R4.west);
    \draw[->, thick, shorten >=15pt] (L3.east) to [out=0, in=180] (R3.west);
\end{tikzpicture}
...
\begin{enumerate}
    ...
    \item Unclear in the context of exogenous policy shocks
\end{enumerate}
...
\begin{enumerate}
    \item Nonprofits drive the hospital industry \scriptsize (\cite{ASPE_2023}) \large

    \vspace{3mm}

    \item Policies often aim to improve the value of health care delivery
...
\begin{frame}{More Details}
    \large
    I leverage several 2012 pay-for-performance initiatives

    \vspace{2mm}
    \begin{wideitemize}
        \item Penalty (reward) for below (above) average \hspace{-8mm} $\underbrace{\text{quality}}_{\text{30-day read \& mort}}$
        \item \textit{Changes the relationship between revenue and quality in objective function}
    \end{wideitemize}
    ...
    \vspace{2mm}
    \begin{wideitemize}
        \item Potentially correlated with hospital's preferences towards revenue versus patient welfare
    \end{wideitemize}
...
\large
    ...
    \begin{itemize}
        \item Top level C-suite employees scraped from 990s
    \end{itemize}
    ...
    \vspace{1mm}
    \begin{wideitemize}
        \item AHA, Hospital Compare, CMS Case Mix Index, HCRIS
    \end{wideitemize}
...
\large
    ...
    \begin{itemize}
        \item No weights
        \item \textbf{Weights based on outcomes pre-2012} 
        \item Coarsened exact matching 
    \end{itemize}
\end{frame}

\begin{frame}{Identification Assumptions}\label{ident}
...
\begin{wideitemize}
    ...
    \item Stability in clinical executives not correlated with incentive change \hyperlink{changes}{\beamergotobutton{analysis}}
    \begin{itemize}
        \item No correlation between 2012 and likelihood of changing propensity to hire clinical executive
        \item No correlation between exposure to policies and likelihood of changing propensity
    \end{itemize}
\end{wideitemize}
\end{frame}
...
\begin{frame}{Mortality Rate}\label{forprofmort}
    \begin{columns}
    ...
    \hyperlink{unobserved}{\beamergotobutton{unobserved}}
\end{frame}
...
\hspace{1em}Penalized for two conditions & 0.50 & 0.47 & 0.38\\
...
\begin{frame}{Average Team Changes Over Time}\label{changes}
\centering 
...
\begin{equation*}\label{eq:change2}
    ...
    \hyperlink{ident}{\beamerreturnbutton{back}}
    
\end{frame}

\begin{frame}{Driven by Unobserved Compositional Differences?}\label{unobserved}
\large
...
\hyperlink{forprofmort}{\beamerreturnbutton{back}}
\end{frame}

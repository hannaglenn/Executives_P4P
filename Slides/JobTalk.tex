...
% Author 
\node[below=1cm] (author) at (title.south){\large Hanna Glenn \\
...
\vspace{2mm}
    ...
    \begin{wideitemize}
        \item Should nonprofits receive the benefits that they do? 
        \item Push to make nonprofits act more like nonprofits
    \end{wideitemize}
    \end{enumerate}
\end{frame}
...
\large
    ...
    \vspace{2mm}
    \begin{wideitemize}
        \item American Hospital Association
...
\large
    \begin{equation*}
    y_{ht} = \beta \left(\mathbb{1}\{\rho \leq \eta\}\text{ x post 2012}\right)_{ht} + \gamma_{h} + \delta_t + \epsilon_{ht}
    \end{equation*}

    \begin{equation*}
    y_{ht} = \beta \left(\mathbb{1}\{\rho > \eta\}\text{ x post 2012}\right)_{ht} + \gamma_{h} + \delta_t + \epsilon_{ht}
    \end{equation*}

    \vspace{10mm}

    \begin{itemize}
        \item $\rho$: fraction of clinical executives
        \item $\eta$: median fraction of clinically trained executives (.2)
    \end{itemize}
\end{frame}

\begin{frame}{Intensive Margin: Readmission Rates}
    \begin{columns}
    \column{0.5\textwidth}
        \centering
        \includegraphics[width=.9\textwidth]{Objects/cont_belowmedread_md_nomd_synth_graph.pdf}
    \column{0.5\textwidth}
        \centering
        \includegraphics[width=.9\textwidth]{Objects/cont_abovemedread_md_nomd_synth_graph.pdf}
    \end{columns}
\end{frame}
...
\vspace{3mm}
\begin{wideitemize}
    \item if true, we should see a difference in response no matter the timing of having a clinical executive
\end{wideitemize}
\end{frame}
...
\large
    \begin{wideitemize}
        \item Not driven by changes in patient population
        \item Driven primarily by pneumonia
    \end{wideitemize}
\end{frame}
...
\Large
    ...
    \textcolor{black}{\faEnvelope \hspace{2mm} hanna.glenn@uq.edu.au}

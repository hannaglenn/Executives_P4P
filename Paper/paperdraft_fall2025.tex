...
\begin{document}
 ...
 \vspace{3mm}
    ...
    \subsection{Clinical CEO}
        ...
        Additionally, I consider the same comparison of clinical and non-clinical leaders, but only for CEOs. Specifically, I examine whether the results hold when focusing solely on the CEO rather than the entire executive team. I compare hospitals led by a clinically trained CEO to those led by a non-clinical CEO using the same synthetic difference-in-differences strategy. I present the average treatment effect and standard errors of this estimation in Table \ref{tab:mainanalysis_ceo}. There is no difference in response when only considering clinical CEOs as the treatment since the effects are statistically insignificant or smaller in magnitude. This highlights the importance of considering executive teams as a whole as opposed to just upper managers. 
   ...
   \import{Tables}{ceo_table.tex}
    ...
    While uncovering that there is some foundational difference in clinical and non-clinical hospital leaders is important in its own right, it is especially relevant to understand why this difference might come about. In this Section, I present several theoretical predictions consistent with the observed difference in quality after pay-for-performance incentives, and  test the predictions empirically. First, I investigate whether clinicians are actively managing hospitals differently or whether they are a signal of underlying preferences of the hospital by leveraging the timing of clinical leaders relative to the incentives in Section \ref{sec:sig_man}. Second, the difference could be driven by the altruism, or patient-centered focus, of clinicians. I investigate this in Section \ref{sec:forprofit}, comparing the response of nonprofits with different leadership teams to the response of for-profit hospitals on average. Third, I consider the altruism versus expertise argument using a heterogeneity analysis of different physician specialties in Section \ref{sec:expertise}. In Section \ref{sec:selection}, I consider whether different types of executives might have different propensities to engage in "gaming" through selective patient practices. Finally, I analyze whether different executives change investment behavior in response to the incentives in Section \ref{sec:investment}.

    \subsection{Signaling versus Managing}\label{sec:sig_man}

    Given that having clinically trained executives does affect hospital response to financial incentives on quality, an important distinction is whether the existing clinical leaders \textit{reveal} the underlying objectives of the hospital, or \textit{manage} the hospital differently. In the main analysis, I limited the sample of nonprofit hospitals to those that don't change their propensity to hire a clinically trained executive over the sample period, which combines any signaling and managing effects. However, under the assumption that hiring or firing these types of executives is not correlated with the pay-for-performance policy changes, such changes can be leveraged to decompose the effect into signaling versus managing. Specifically, if the difference is driven by underlying preferences of the hospital, the observed difference in quality will still hold whether the hospital had a clinical executive at any point in time, even if they were not a leader in 2012 when the policies took effect. Alternatively, if the observed difference is driven by managing, then the difference should only be observed when there is a clinician in leadership during the policy enactment. Since this decomposition depends heavily on the assumption that clinical executive hiring is not endogenous, I first investigate leadership changes in response to the policy change. I provide details of the decomposition estimation in Section \ref{sec:decomp}
...
\end{center}
 \end{figure}
    ...
    The first prediction of the model is that, all else held equal, hospitals that value profit will have a lower quality than those who value patient welfare in a fee-for-service setting. Second, a hospital's change in quality once pay-for-performance is enacted depends on how much weight the hospital places on profit vs. patient welfare. Particularly, hospitals with higher weight on profit respond more than hospitals with higher weight on patient welfare. Therefore, factors that affect weight $\alpha$ will be correlated with choice of quality, and with how quality changes under new pay-for-performance incentives. In this paper, I test the hypothesis that clinical experience on a hospital's leadership team is a factor that is correlated with weight $\alpha$, and thus affects response to pay-for-performance incentives. To investigate this hypothesis empirically, I merge all general for-profit hospitals in the AHA survey to my sample of nonprofit hospitals. I then compare readmission and mortality rates of always clinical executives and never clinical executives to for-profit hospitals after the incentive change. 
    
    While I do not have information on leadership teams of for-profit hospitals, on average, they have revealed through their ownership status that they more heavily weigh profit, and therefore have an $\alpha$ close to 1. This is not inconsistent with previous work positing that nonprofits and for-profits act similarly on average (\cite{duggan2002hospital}). In fact, my data is consistent with this idea when nonprofits are not broken down by leadership teams. However, comparing different executive team behavior to for-profit behavior helps reveal how profit driven certain leadership teams are, which could actually help us understand why there are similarities between nonprofits and for-profits on average. I estimate the weighted synthetic difference-in-differences version of
    ...
    $c(\theta)(1 + \lambda)$ is the cost of quality, increasing and convex in $\theta$, scaled by expertise, $u(\theta)$ is patient welfare, increasing and concave in $\theta$, $\alpha \in [0,1]$ is the weight on profit relative to patient welfare. The hospital's first-order condition under fee-for-service and pay-for-performance is then:
    ...
    I now want to compare the change in $\theta$ when going from fee-for-service to pay-for-performance depending on different levels of $\lambda$. 
    \begin{align}
    ...
    \citeauthor{gupta2021impacts} (\citeyear{gupta2021impacts}) finds that a large portion of hospital readmission reductions are due to hospitals being less willing to readmit patients from their own hospital, while they will still readmit patients coming from other hospitals. Given the differential response to the HRRP by clinical and non-clinical hospital leaders, the natural question arises as to whether there is a differential propensity to game the system, as considered in previous work (\cite{gupta2021impacts}). While I do not have patient level data, I can test empirically whether the overall patient population of hospitals changes differentially for clinician or non-clinician leaders. 

    I use three outcomes to test this hypothesis. First, the CMS Case Mix Index files provide an aggregate measure of patient complexity for each hospital in each year. A higher index implies the patient population in the hospital is more complex in their treatment requirement. Additionally, I consider the population of patients treated in the hospital with public insurance, either Medicare or Medicaid. These outcomes will indicate if certain types of leadership teams attempt to cost shift relative to others after the penalties or rewards took effect. 

    I estimate the same synthetic difference-in-differences equation discussed in Section \ref{sec:clinical}. The results of these estimations are presented in Figure \ref{fig:selection}. All of the treatment effects in these estimations are insignificant and null in magnitude, suggesting that there is no evidence of the overall patient population of hospitals changing differentially based on leadership team. This does not mean that there is no change in specific readmission behavior as found in \citeauthor{gupta2021impacts} (\citeyear{gupta2021impacts}), but a limitation of this data only allows me to look at the aggregate patient population as opposed to specifically readmissions. 

    \begin{figure}[ht!]
     \caption{Patient Population as Outcomes}
     \centering
     \begin{subfigure}[b]{0.45\textwidth}
         \centering
         \caption{Case Mix Index}
         \includegraphics[width=\textwidth]{Objects/cmi_md_nomd_synth_graph.pdf}
         \label{fig:cmi}
     \end{subfigure}
     \hfill
     \begin{subfigure}[b]{0.45\textwidth}
         \centering
         \caption{Medicare Percent Discharges}
         \includegraphics[width=\textwidth]{Objects/mcare_md_nomd_synth_graph.pdf}
         \label{fig:mcare}
     \end{subfigure}%
        \begin{subfigure}[b]{0.45\textwidth}
         \centering
         \caption{Medicaid Percent Discharges}
         \includegraphics[width=\textwidth]{Objects/mcaid_md_nomd_synth_graph.pdf}
         \label{fig:mcaid}
     \end{subfigure}
        \label{fig:selection}
    \end{figure}
    ...
    Lastly, I examine whether clinician versus non-clinician leaders respond differentially in terms of their investment strategy after the policies took effect. Ideally, I would have more granular information within the hospital to observe differences in management. That is a limitation of the publicly available hospital-level data that I utilize here. I use the same synthetic difference-in-differences estimation strategy to compare clinical leaders with non-clinical leaders before and after 2012. The variables I use as outcomes are the number of full time nurses hired (from AHA), movable equipment purchases, fixed equipment purchases, labor costs, building purchases, and total operating expenses (all from HCRIS). For several of these variables, over 90\% of observations are missing in 2010. Therefore, I remove this year for the relevant variables. The results of these estimations are shown in Table \ref{tab:investment}, and do not present any evidence that hospitals with different leadership teams are changing their investment strategies differentially as a result of the policies. 
